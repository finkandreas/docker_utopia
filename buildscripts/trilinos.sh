...
DEPRECATED_OPTIONS="-DKokkos_ENABLE_DEPRECATED_CODE=OFF"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DTrilinos_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DTpetra_ENABLE_DEPRECATED_CODE=OFF"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DTpetra_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DBelos_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DEpetra_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DEpetraExt_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DIfpack2_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DIfpack2_ENABLE_DEPRECATED_CODE=OFF"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DIntrepid_ENABLE_DEPRECATED_KOKKOS_CODE=OFF"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DMueLu_ENABLE_DEPRECATED_CODE=OFF"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DMueLu_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DPanzer_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DPhalanx_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DRTOp_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DSTK_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DTeuchos_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DThyra_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DXpetra_ENABLE_DEPRECATED_CODE=OFF"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DClaps_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DGlobiPack_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DOptiPack_HIDE_DEPRECATED_CODE=ON"
DEPRECATED_OPTIONS="${DEPRECATED_OPTIONS} -DTrios_HIDE_DEPRECATED_CODE=ON"
#DEPRECATED_OPTIONS=""  # unset for now, since utopia does not compile with all deprecated code disabled
...
cmake -DCMAKE_INSTALL_PREFIX=${INSTALLDIR} -DCMAKE_VERBOSE_MAKEFILE=ON -DCMAKE_BUILD_TYPE=${BUILD_TYPE} \
      -DCMAKE_C_FLAGS="-fopenmp" -DCMAKE_Fortran_FLAGS="-fopenmp" -DCMAKE_CXX_FLAGS="-std=c++14 -O3 ${MARCH} ${CXX_CUDA_FLAGS}" \
      ...
      -DPETSC_INCLUDE_DIRS=${PETSC_DIR}/include -DPETSC_LIBRARY_DIRS="${PETSC_DIR}/lib" -DPETSC_LIBRARY_NAMES="petsc" \
      ...
      -DTrilinos_ENABLE_Belos=ON -DBelos_Tpetra_Timers=ON -DBelos_ENABLE_TSQR=ON \
      -DTrilinos_ENABLE_Epetra=ON -DEpetraExt_USING_HDF5=ON -DEpetra_ENABLE_Fortran=OFF -DEpetra_ENABLE_THREADS=ON -DEpetra_ENABLE_WARNING_MESSAGES=ON \
      -DTrilinos_ENABLE_Ifpack=ON -DTrilinos_ENABLE_Ifpack2=ON -DIfpack2_ENABLE_Experimental_KokkosKernels_Features=OFF -DIfpack2_ENABLE_IFPACK2_TIMER_BARRIER=ON \
      -DTrilinos_ENABLE_Intrepid=ON -DTrilinos_ENABLE_Intrepid2=ON \
      -DTrilinos_ENABLE_Kokkos=ON -DKokkos_CXX_STANDARD=14 -DKokkos_ENABLE_DEBUG=OFF -DKokkos_ENABLE_Debug_Bounds_Check=OFF -DKokkos_ENABLE_MPI=ON \
      ...
      -DTrilinos_ENABLE_Tpetra=ON -DTrilinos_ENABLE_TpetraCore=ON -DTpetra_THROW_Efficiency_Warnings=ON -DTpetra_INST_OPENMP=ON -DTpetra_INST_SERIAL=ON -DTpetra_ASSUME_CUDA_AWARE_MPI=ON \
      ...
      -DTPL_ENABLE_Dakota=OFF -DTPL_ENABLE_Eigen=OFF -DTPL_ENABLE_HYPRE=ON -DTPL_ENABLE_MPI=ON -DTPL_ENABLE_PETSC=OFF -DTPL_ENABLE_y12m=OFF -DTPL_FIND_SHARED_LIBS=ON \
      ${DEPRECATED_OPTIONS} \
      ${EXAMPLES_TESTS} \
      ${CMAKE_CUDA_OPTS} \
      ${SRCDIR}
...
if [[ -n ${BUILD_WITH_CUDA_SUPPORT} ]] ; then
  cp ${BUILDSCRIPT_DIR}/nvcc_wrapper ${INSTALLDIR}/bin/nvcc_wrapper
  find ${INSTALLDIR}/lib/cmake -name '*.cmake' -exec sed -i -e "s|${BUILDSCRIPT_DIR}/nvcc_wrapper|${INSTALLDIR}/bin/nvcc_wrapper|" {} \;
fi
